...
from training_contracts import (
    matched_initialization, EpochDataset, start_epoch, optimizer_groups,
    accumulation_weight, optimizer_update, ema_evaluation, immutable_config,
    checkpoint_contract, resume_training, selection_metadata, validate_eval_index,
)
...
class SwinWithHooks(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        def tokens_to_spatial(tokens: torch.Tensor) -> torch.Tensor:
            ...
            H = W = int(N ** 0.5)
            if H * W != N:
                raise RuntimeError(f"Non-square token count N={N}")
        ...
        tokens3 = s2
        # All token arms start from the pretrained final LayerNorm output.
...
class TeaLeafDataset(Dataset):
    ...
    def __init__(
        ...
        # Canonical discovery/order must exactly match torchvision ImageFolder.
        # This is also the ordering used by the frozen evaluation index.
        canonical = ImageFolder(self.root_dir)
        if list(canonical.classes) != list(self.CLASS_NAMES):
            raise ValueError(
                "Unexpected class layout/order:\n"
                f"  expected: {self.CLASS_NAMES}\n"
                f"  got:      {canonical.classes}"
            )
        self.classes = list(canonical.classes)
        self.class_to_idx = dict(canonical.class_to_idx)
        self.idx_to_class = {i: c for c, i in self.class_to_idx.items()}
        self.samples: List[Tuple[Path, int]] = [
            (Path(path), int(label))
            for path, label in canonical.samples
        ]
        if not self.samples:
...
class TCCATrainer:
    ...
    def _assert_param_budget(self):
        ...
        if m.arch == "rgb":
            for attr in ("tcca", "adapter", "eca", "color_encoder", "color_proj4"):
                if hasattr(m, attr):
                    raise RuntimeError(f"arch='rgb' unexpectedly contains {attr}")
        ...
        if expected is not None:
            if token_params != expected:
                raise RuntimeError(f"PARAM BUDGET MISMATCH: {token_params} != {expected}")
    ...
    def _build_loaders(self):
        ...
        test_ds  = TeaLeafDataset(root / "test",  None,
                                  ...
                                  augment=False, hue_jitter=hj)
        train_ds = EpochDataset(train_ds, self.cfg.seed)
    ...
    def train_one_epoch(self, epoch: int) -> Dict:
        ...
        for i, batch in enumerate(pbar, start=1):
            ...
            self.scaler.scale(loss * accumulation_weight(
                self.train_loader, i, accum, x.size(0))).backward()
    ...
    def resume_from(self, ckpt_path: Path):
        resume_training(self, ckpt_path, self._base_model(), load_state_dict_robust)
    ...
    def fit(self):
        ...
        test_m.update(selection_metadata(self, best_ckpt, used_ema))
